...
class BaseProcessor:
    def __init__(self,data,portfolio:str,metric:str,date_choices:dict,obs_period:int):
        self.data = data
    ...
    def func_forms_df(self,df,dfs):
        df1 = df[df['portfolio'] == self.portfolio].copy()
        dfs[self.portfolio+'_df'] = df1.pivot_table(index=['vintage','scenario'],columns='start_ind',values=self.metric,aggfunc='sum').reset_index()
        return dfs[self.portfolio+'_df']
    
    def func_forms(self,vin:str,dfs):
        df_port = dfs[self.portfolio+'_df']
    ...
    def base_process(self,*,date_choices: dict = None):
        ...
        # filter out forecast row if it is completely NULL since they represent actuals due to the start of observation period
        cols_to_replace = [c for c in df_stacked.columns if c not in ['vintage','scenario']]
        df_stacked = df_stacked[~((df_stacked['scenario'] == 'forecast') & (df_stacked[cols_to_replace].isnull().all(axis=1)))]
